...
function createTileUrl(z: number, x: number, y: number) {
  const maxTile = 2 ** z;
...
export function LiveOsmMap({
    ...
    for (let x = minTileX; x <= maxTileX; x += 1) {
      for (let y = minTileY; y <= maxTileY; y += 1) {
        ...
        tiles.push({
          ...
          url: createTileUrl(effectiveZoom, x, y),
  ...
  return (
    <View style={styles.container} onLayout={onLayout}>
      ...
      {showAttribution && <Text style={styles.attribution}>© OSM contributors</Text>}
    </View>
  );
}
